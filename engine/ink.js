...
try {
  const old = JSON.parse(localStorage.getItem('gnn_ink_tool') || '{}');   // 예전 도구 설정에서 손가락 설정만 이어받기
  if (old.fingerNav != null) tool.fingerNav = old.fingerNav;
  if (old.penSeen != null) tool.penSeen = old.penSeen;
  const v2 = JSON.parse(localStorage.getItem('sdt_ink_tool2') || '{}');
...
const saveTool = () => { try { localStorage.setItem('sdt_ink_tool2', JSON.stringify(tool)); } catch (e) { /* 무시 */ } };
...
window.SDTInk = { Layer, Toolbar, tool, saveTool, getPage, putPage, delPage, keys, all, preview, Sync, setUser, recognize };
})();
